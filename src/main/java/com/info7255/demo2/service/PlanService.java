package com.info7255.demo2.service;

import com.info7255.demo2.exception.*;
...
//        PlanService.class.getResourceAsStream("src/main/resources/static/jsonschema.json"))
            ...
            try {
                ...
                throw new ExpectationFailed("Enter correct input! The correct input should be " + e.getMessage());
            }
    }
        ...
        if(!tokenService.validateToken(token)) throw new BadRequest("Token is expired");
            ...
            if (etag != null) {
                if (EtagMap.getEtags().containsKey(id+"g")) {
                    if (EtagMap.getEtags().get(id+"g").equals(etag)) throw new Notmodified("Data has not been updated since last time!");
                    else{
        ...
        if (EtagMap.getEtags().containsKey(planId + "p")) {
                ...
                if (jedis.get(planId) != null) {
                    if ((deleteData(planId)) > 0) {
                        ...
                        EtagMap.getEtags().put(planId + "pu", etag);
